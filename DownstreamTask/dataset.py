...
try:
    ...
    # Older PyTorch or class not available
    pass

try:
    # Add additional classes that may be needed
...
class ChunkedGeneGraphDataset(Dataset):
    """
    Chunked gene graph dataset for handling very large datasets.
    Extends Dataset and loads data chunks on demand instead of keeping everything in memory.
    """
    def __init__(self, h5ad_paths, edge_indices=None, edge_attrs=None, split='train',
        ...
        # Process other parameters (same as original)
    ...
    def _process_edge_param(self, param, num_files):
        ...
        if param is None:
            ...
            if len(param) == num_files:
                ...
                raise ValueError(f"Edge parameter list length mismatch")
        else:
            raise ValueError("Invalid edge parameter format")
    
    def _process_to_chunks(self):
        """Process data and save as chunks."""
        import gc
        import json
        ...
        for file_idx, h5ad_path in enumerate(self.h5ad_paths):
            ...
            for i in range(num_samples):
                ...
                # Concatenate features: [cell_expr, g_mean, g_var] -> [num_genes, 3]
                # x = torch.cat([cell_expr, g_mean, g_var], dim=1)
                x = cell_expr  # Use only single-cell expression as node features
                ...
                # Check whether to save the current chunk
    ...
    def __getitem__(self, idx):
        ...
        # Determine which chunk the sample belongs to
...
class ChunkAwareRandomSampler:
    """Random sampler that respects chunk boundaries."""
