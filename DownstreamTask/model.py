...
class GINEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, num_layers=3, dropout=0.1):
        ...
        mlp = nn.Sequential(
            ...
            nn.LayerNorm(hidden_dim),  # Use LayerNorm for pre-activation normalization
            nn.ReLU(),
            nn.Dropout(dropout),
        ...
        for i in range(1, num_layers - 1):
            ...
            nn.init.xavier_uniform_(skip_conn.weight, gain=0.5)  # Small gain for stability
        ...
        nn.init.constant_(self.node_fc_logvar.bias, -2)  # Initialize to small variance
    ...
    def forward(self, x, edge_index, batch):
        ...
        # Store original expression values for weighting (numerical stability)
        ...
        for i in range(1, self.num_layers - 1):
            identity = out  # Save input for residual
            out = self.bns[i](self.convs[i](out, edge_index))
            out = F.relu(out)
            out = self.dropouts[i](out)
            
            # Improved skip connection - weighted combination
            if i > 0 and len(skip_outs) > 0:
                skip_proj = self.skip_connections[i-1](skip_outs[i-1])
                # Use learnable weight combination
                out = 0.7 * out + 0.3 * skip_proj + 0.1 * identity  # Multi-level connection
        ...
        node_logvar = torch.clamp(self.node_fc_logvar(out), -10, 10)  # Clamp logvar range
        
        # Improved graph-level representation computation
        # Use softmax-normalized weights for stability
        expr_weights = torch.abs(original_expr) + 1e-6
        
        # Compute graph-level representation per batch item
        ...
        for batch_idx in unique_batch_indices:
            ...
            if len(curr_weights) > 1:
                # Temperature parameter to control weight distribution sharpness
                temperature = 0.5
            ...
            # Weighted average with extra numerical stability
            graph_mu_i = torch.sum(curr_node_mu * curr_weights_norm, dim=0, keepdim=True)
            
            # Use log-sum-exp trick for logvar numerical stability
        ...
        # Final numerical stability clamp
...
class GINVAE(nn.Module):
    ...
    def _print(self, message, force=False):
        """Helper method to control output."""
        if self.verbose or force:
            print(message)
            
    def _tqdm_write(self, message, force=False):
        """Write message via tqdm.write to avoid interfering with progress bars."""
        if self.verbose or force:
            tqdm.write(message)
    ...
    def process_and_save_reference_dataset(self, x_ref_dataset, save_path, max_samples_per_batch=1000, 
        ...
        with torch.no_grad():
            ...
            for batch_idx in batch_progress:
                ...
                # Show sample-level progress only in verbose mode
    ...
    def load_and_compute_bio_context(self, save_path, group_type=None, group_name=None, max_ptrb_samples=None):
        ...
        if use_chunked_saving:
            ...
            # Show chunk loading progress with tqdm
            ...
            if group_type is not None and group_name is not None:
                ...
                # Show filtering progress with tqdm
        ...
        # Show control sample processing progress with tqdm
        ctrl_graph_z_list = []
        ...
        # Show perturb sample processing progress with tqdm
        ptrb_graph_z_list = []
    ...
    # Convenience method: complete pipeline for processing reference dataset
    def process_reference_dataset(self, x_ref_dataset, save_path=None, group_type=None, group_name=None, 
        ...
        """
        Unified reference data processing: processes and saves if file does not exist, then computes biological context.
