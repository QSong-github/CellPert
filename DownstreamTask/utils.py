...
def set_seed(seed):
    ...
    # PyTorch 1.8+ requires an additional setting
...
class BatchSizeWarmupScheduler:
    ...
    def step_epoch(self):
        if self.current_epoch < self.warmup_epochs:
            # Warmup phase: start with a very small lr
...
def process_variable_size_batches_with_mask(all_recon, all_x, all_masks):
    """
    Process variable-size batches, preserving each sample's full node sequence for sample-level metric computation.
    """
