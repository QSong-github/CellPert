...
def string_map_ids(genes: List[str], species: int = 9606, sleep=0.34) -> Dict[str, str]:
    """
     gene symbol to STRING  protein ID。
...
def string_get_network(
    ...
    """
     STRING(pairwise edges + combined_score)
...
def build_corr_edges(
    ...
    """
     (cells x genes) knn-> [gene_i, gene_j, weight].
    - topk: 
...
def build_gene_graph_from_h5ad(
    ...
    return:
      ...
      - gene_order (list[str]) 
      - edges_df (DataFrame) (gene_i, gene_j, weight, source)
    """
